...
function drawChart(svgElement, data, width, height, ticker){
    ...
    // Specs for the axes
    const xExtents = d3.extent(data.map((d) => 1.1*d.tSNE_x));
    const yExtents = d3.extent(data.map((d) => 1.1*d.tSNE_y));
    ...
    const dots = svg.append('g')
        ...
        .attr('r', 5)
        .attr('fill', 'teal')
        .attr('class', (d) => `tsne-point tsne-point-${sectorMap[d.Ticker]}`)
        .attr('id', (d) => `tsne-point-${d.Ticker}`)
    ...
    const dotLabels = svg.append('g')
        ...
        .attr('y', (d) => yScale(d.tSNE_y))
        .attr('dy', -18)
        .attr('visibility', 'hidden')
        .attr('text-anchor', 'middle')
        .attr('class', 'tsne-label')
        .attr('id', (d) => `tsne-label-${d.Ticker}`)
        .attr('clip-path', 'url(#clip)');
    ...
    const zoom = d3.zoom()
        ...
        .on('zoom', (e) => {
            ...
            // Redraw dots
            dots
                .attr('cx', (d) => newScaleX(d.tSNE_x))
                .attr('cy', (d) => newScaleY(d.tSNE_y));
            dotLabels
                .attr('x', (d) => newScaleX(d.tSNE_x))
                .attr('y', (d) => newScaleY(d.tSNE_y))
        });

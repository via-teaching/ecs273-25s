...
export function Scatter(selectedStock) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const filepath = "../../data/tsne.csv"
      try {
      const csvData = await d3.csv(filepath, d => {
        return {ticker: d.ticker, x: +d.x, y: +d.y, class: d.class};
      });
        setPoints(csvData);
      } catch (error) {
...
function drawChart(containerRef, svgElement, points, width, height, selectedStock) {
  ...
  const classes = Array.from(new Set(points.map(d => d.class)));

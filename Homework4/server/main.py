...
# # MONGO_URI (connection string) = mongodb+srv://alyssayee:123@cluster0.kdapobk.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0
...
# db = client.stock_ayee # please replace the database name with stock_[your name] to avoid collision at TA's side
...
#     cursor = news_collection.find({"Stock": stock_name}).sort("Date", 1) # took out .sort
#     news_list = await cursor.to_list(length=100) # length=None
#     return news_list

# @app.get("/stock/{stock_name}", response_model=StockModelV2)
# async def get_stock(stock_name: str):
#     collection = db.get_collection("stock_data_v2")
#     doc = await collection.find_one({"name": stock_name})
#     return doc
...
# #     doc = await collection.find_one({"Stock": stock_name})
# #     return doc
# @app.get("/tsne/", response_model=list[tsneDataModel])
# async def get_tsne(stock_name: str = None): #stf = XOM
...
#             "y": doc["TSNE2"]
#         }
#         tsne_list.append(tsne_entry)

#     print(f"Returned {len(tsne_list)} tsne records")
#     return tsne_list

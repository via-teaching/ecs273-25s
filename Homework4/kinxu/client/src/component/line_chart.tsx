...
function drawLineChart(svgElement: SVGSVGElement, data, width: number, height: number) {
    var dates: string[] = [];
    var prices: number[] = [];
    var opens: price[] = [];
    var lows: price[] = [];
    ...
    for (var i = 0; i < data["date"].length; i++) {
        var date = parseDate(data["date"][i]);
        var open = Number(data["Open"][i]);
        var low = Number(data["Low"][i]);
        var high = Number(data["High"][i]);
        var close = Number(data["Close"][i]);
    ...
    const minXAxisWidth: number =  dates.length * 100;
...
export function LineChart() {
    ...
    useEffect(() => {
        ...
        .then((res) => res.json())
        .then((data) => {
            const stock_data = data["stock_series"]
            if (width && height && ! isEmpty(stock_data)) {
                drawLineChart(svgRef.current, stock_data, width, height);
            }
        });

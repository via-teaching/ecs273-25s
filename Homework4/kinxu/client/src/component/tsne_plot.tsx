...
function drawScatterPlot(svgElement: SVGSVGElement, data, width: number, height: number, ticker: string) {
    var x: number[] = [];
    var y: number[] = [];
    var sectors: string[] = [];
    ...
    pointsGroup
    .selectAll("text")
    .data(data.filter((d) => d["Stock"] == ticker))
    .enter()
    .append("text")
...
async function getTsneData(tickers) {
    var tsneData = await Promise.all(
        tickers.map((stockTicker) => {
            return fetch("http://localhost:8000/tsne/" + stockTicker)
            .then((res) => res.json())
        })
    )
        ...
        const resizeObserver = new ResizeObserver(
            debounce((entries: ResizeObserverEntry[]) => {
                for (const entry of entries) {
                    ...
                    .then((data) => {
                        (async() => {
                            var tsneData: Point[] = await getTsneData(data["tickers"])
                            if (width && height && !isEmpty(tsneData)) {
                                drawScatterPlot(svgRef.current, tsneData, width, height, ticker)
                            }
                        })();

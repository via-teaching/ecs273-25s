...
export function Figure1_linechart() {
  ...
  const sync_data_to_chart = async (stock) => {
    ...
    const response = await fetch(`http://localhost:8000/stock/${stock}`);

    const data = await response.json();

    const mongo_stock_data = [];
    
    // Loop through the arrays and create objects for each data point
    for (let i = 0; i < data.date.length; i++) {
      mongo_stock_data.push({
    ...
    // Sort by date
    mongo_stock_data.sort((a, b) => a.date - b.date);
    ...
    const resizeObserver = new ResizeObserver(
      debounce((entries) => {
        for (const entry of entries) {
          ...
          if (width && height && !isEmpty(mongo_stock_data)) {
            drawChart(svgRef.current, mongo_stock_data, stock, width, height);
          }
        }
      }, 100)
    );
